...
function CompanyLoginPageContent() {
    ...
    try {
      ...
      try {
        const checkResponse = await fetch("/api/auth/login", {
          ...
          body: JSON.stringify({ username, password, accountType: "company" }),
        });
        ...
        if (result.error) {
          ...
          switch (result.error) {
            ...
            case "USERNAME_PASSWORD_REQUIRED":
              errorMessage = "Por favor, preencha username e senha.";
              break;
        ...
        <div className="bg-muted rounded-(--radius) border p-3">
          <p className="text-black text-center text-sm">
            ...
            <Button asChild variant="link" className="px-2">
              <Link href="/feed/auth/login">Fazer login como usuário</Link>
            </Button>
          </p>
        </div>
      </form>
